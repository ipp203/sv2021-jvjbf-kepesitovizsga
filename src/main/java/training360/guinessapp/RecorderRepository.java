...
public interface RecorderRepository extends JpaRepository<Recorder, Long> {
    ...
    @Query("select new training360.guinessapp.dto.RecorderShortDto(r.name, r.dateOfBirth) from Recorder r " +
            "where r.name like 'B%' or r.name like '%e%' " +
            "order by r.dateOfBirth desc")
    List<RecorderShortDto> findAllFilteredByLettersOrderedByDate();
}
